...
export class AppComponent implements OnInit{
  ...
  onSubmit(myForm:FormGroup){
      ...
      if(myForm.value.username==''||myForm.value.city==''||myForm.value.gender=='')
        alert("complete the form !")
        else {
        console.log(myForm.value);
        let dataFromLocalStorage=JSON.parse(localStorage.getItem('AppData')||'{}');
        

        if(dataFromLocalStorage.length==undefined)
        {  
          this.ArrayForAppData.push(myForm.value);
          localStorage.setItem('AppData',JSON.stringify(this.ArrayForAppData));
        } 
        else
        { 
          ...
          localStorage.setItem('AppData',JSON.stringify(dataFromLocalStorage));
